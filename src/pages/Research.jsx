...
export default function Research() {
  ...
  useEffect(() => { const unsub = base44.entities.TechProgress.subscribe(() => loadProgress()); return unsub; }, [loadProgress]);
  useCycleRefresh(empire?.last_tick_date, loadProgress);

  const statusMap = useMemo(() => (progress ? deriveStatuses(progress) : {}), [progress]);
  const completedIds = useMemo(() => RESEARCH_TEST_MODE
    ? new Set(TECH_TREE.map((t) => t.id))
    : new Set(Object.entries(progress || {}).filter(([, r]) => r?.status === 'completed').map(([id]) => id)), [progress]);
  ...
  const beginResearch = async (techId) => {
    ...
    try {
      ...
      if (res?.data?.record) setProgress((current) => ({ ...current, [res.data.record.tech_id]: res.data.record }));
    ...
    <div className="min-h-screen bg-[#02070d] text-slate-100">
      <div className="max-w-[1600px] mx-auto px-2 sm:px-4 lg:px-6 py-3 lg:py-4 pb-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_310px] gap-2 mt-2">
          <div className="relative overflow-hidden rounded-xl border border-cyan-400/20 bg-[#030b13] shadow-[inset_0_0_50px_rgba(8,47,73,0.25)]"><TechCanvas statusMap={statusMap} edges={edges} layout={layout} selectedId={selectedId} onSelect={setSelectedId} visibleIds={filteredIds} /></div>
