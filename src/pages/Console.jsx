    ...
    <div className="max-w-6xl mx-auto px-4 md:px-8 py-6 md:py-8">
        ...
        {RESOURCES.slice(5).map((r) => {
          ...
          return (
            <div key={r.key} className="glass-panel rounded-lg px-3 py-2 flex items-center gap-3">
              <Icon className={`w-4 h-4 ${r.color}`} />
              <div className="min-w-0">
                <p className="text-[9px] uppercase tracking-widest text-muted-foreground truncate">{r.key === 'research_points' ? 'Research Pts' : r.label}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-5">
        ...
        <section>
          ...
          <ActiveResearchPanel empire={empire} />
        </section>
      </div>
