  ...
  return <div className="space-y-1.5">{researching.map((rec) => {
    ...
    return <div key={rec.id} className="glass-panel rounded-lg px-3 py-2">
      <div className="flex items-center gap-2.5">
        ...
        <div className="flex-1 min-w-0"><h3 className="font-heading text-xs tracking-wide text-white uppercase leading-tight truncate">{tech ? tech.name : rec.tech_id.replace(/_/g, ' ')}</h3><p className="font-mono text-[9px] uppercase tracking-widest text-slate-500">{Math.floor(invested).toLocaleString()}/{Math.floor(required).toLocaleString()} RP · {Math.floor(frac * 100)}% · {Math.floor(remaining).toLocaleString()} left · ETA {etaMs === null ? 'idle' : formatDuration(etaMs)}</p></div>
