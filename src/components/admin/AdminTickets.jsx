...
export default function AdminTickets() {
  ...
  // Mark all tickets viewed when the component mounts (clears the blinking
  // alert) by setting last_admin_view_date on any ticket with unseen player
  // activity.
  useEffect(() => {
    if (!tickets) return;
  ...
  const handleReply = async (ticket) => {
    ...
    try {
      ...
      await base44.entities.SupportTicket.update(ticket.id, {
        messages: [...ticket.messages, { author_role: 'admin', author_name: 'Admin', body, date: now }],
      });
    ...
    <div>
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-heading text-sm tracking-[0.3em] text-cyan-200/80 uppercase">Support Tickets</h2>
          {unreadCount > 0 && (
            <span className="flex items-center gap-1 text-[10px] font-mono uppercase tracking-widest text-rose-300 border border-rose-400/40 rounded px-2 py-0.5 animate-pulse-glow">
              <AlertCircle className="w-3 h-3" /> {unreadCount} new
            </span>
          )}
        </div>
        <div className="flex gap-2 ml-auto">
          <Select value={filterStatus} onValueChange={setFilterStatus}>
            <SelectTrigger className="w-36 bg-background/40"><SelectValue /></SelectTrigger>
            <SelectContent>{STATUSES.map((s) => <SelectItem key={s.key} value={s.key}>{s.label}</SelectItem>)}</SelectContent>
          </Select>
          <Select value={filterCategory} onValueChange={setFilterCategory}>
            <SelectTrigger className="w-40 bg-background/40"><SelectValue /></SelectTrigger>
      ...
      {filtered.length === 0 ? (
        <div className="glass-panel rounded-2xl p-6 text-center">
          <p className="text-sm text-muted-foreground font-body">No tickets match these filters.</p>
        </div>
      ) : (
              ...
              <div key={t.id} className="glass-panel rounded-lg overflow-hidden">
                <button onClick={() => setOpenId(isOpen ? null : t.id)} className="w-full flex items-center gap-3 p-3 text-left hover:bg-cyan-400/5 transition-colors">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      {hasUnread && <span className="w-2 h-2 rounded-full bg-rose-400 animate-pulse-glow shrink-0" />}
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">{fmtDate(t.last_player_activity_date || t.created_date)}</p>
                  </div>
                ...
                {isOpen && (
                  <div className="px-3 pb-3 space-y-3">
                    {t.category === 'name_change' && t.requested_name && (
                      <div className="glass-panel rounded-lg p-2.5 border border-cyan-400/25">
                        <p className="text-[9px] uppercase tracking-widest text-muted-foreground">Requested Name</p>
                        <p className="font-heading text-cyan-100 uppercase tracking-wide">{t.requested_name}</p>
                      </div>
                    )}
                    <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                      {t.messages?.map((m, i) => (
                        <div key={i} className={`flex ${m.author_role === 'admin' ? 'justify-end' : 'justify-start'}`}>
                          <div className={`max-w-[85%] rounded-lg p-2.5 ${m.author_role === 'admin' ? 'bg-rose-400/10 border border-rose-400/20' : 'bg-cyan-400/15 border border-cyan-400/25'}`}>
                            <p className="text-[9px] uppercase tracking-widest text-muted-foreground mb-0.5">
                              {m.author_name} · {fmtDate(m.date)}
                            </p>
                    ...
                    <div className="flex gap-2 flex-wrap">
                      ...
                      {t.category === 'name_change' && (
                        <Button size="sm" onClick={() => handleAction(t, 'name_change')} disabled={busy[t.id] === 'action' || t.status === 'resolved'} className="font-heading tracking-widest uppercase text-xs">
                          {busy[t.id] === 'action' ? <Loader2 className="w-3 h-3 animate-spin" /> : <CheckCircle2 className="w-3 h-3" />} Approve Name
                        </Button>
                      )}
                    ...
                    <div className="flex gap-2">
                      ...
                      />
                      <Button size="icon" onClick={() => handleReply(t)} disabled={busy[t.id] === 'reply' || !(replyText[t.id] || '').trim()}>
                        {busy[t.id] === 'reply' ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}
                      </Button>
                    </div>
                  </div>
                )}
